...
function blank(){return{schema:'mir2027.state.v2',version:VERSION,revision:0,updatedAt:0,preferences:{dailySize:10,dailyMode:'questions',includeObservations:false,fontScale:1,timezone:'Europe/Madrid'},attempts:[],sessions:{},schedule:{},studySchedule:{},notes:{},marks:{},favorites:[],reports:[],activeSession:null,readPositions:{}};}
function normalize(s){const v=Object.assign(blank(),s||{});v.preferences=Object.assign(blank().preferences,s?.preferences||{});for(const k of ['sessions','schedule','studySchedule','notes','marks','readPositions'])if(!v[k]||Array.isArray(v[k])||typeof v[k]!=='object')v[k]={};for(const k of ['attempts','favorites','reports'])if(!Array.isArray(v[k]))v[k]=[];for(const id of v.favorites)if(!v.marks[id])v.marks[id]={value:true,at:0};v.preferences.dailySize=Math.max(1,Math.min(50,Math.floor(Number(v.preferences.dailySize)||10)));v.preferences.dailyMode=v.preferences.dailyMode==='mixed'?'mixed':'questions';v.preferences.includeObservations=v.preferences.includeObservations===true;v.schema='mir2027.state.v2';v.version=VERSION;return v;}
